<x-app-layout>
    <div class="font-bold text-2xl text-secondary">
        Edit Sliders - {{ $slider->title }}
    </div>
    <div class="my-6 w-full border-b border-gray-200"></div>
    <form action={{ route('homeslider.update', $slider->id) }} method="POST" enctype="multipart/form-data">
        @csrf
        @method('PATCH')
        ...
        <div class="flex items-center mb-6">
            ...
            <x-text-input id="title" class="block w-3/5" type="title" name="title" required
                autocomplete="current-title" value="{{ $slider->title }}" />
